...
func (tf *transferFlags) run(cmd *cobra.Command) error {
	...
	// Resolve context name mapping from the mapping file (optional).
	// Context name overrides live in the Projects map under keys without "/".
	var ctxMapping map[string]string
	if tf.mappingPath != "" {
		mapping, merr := manifest.LoadMapping(tf.mappingPath)
		if merr != nil {
			return fmt.Errorf("loading mapping: %w", merr)
		}
		ctxMapping = make(map[string]string)
