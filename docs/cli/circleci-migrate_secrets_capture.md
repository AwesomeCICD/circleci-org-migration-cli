...
```
  ...
  -o, --output string                 Path to the secret bundle to write/append (default "secrets.json")
      ...
      --ssh-keys                      Extract additional SSH private keys for projects that have cataloged SSH keys in the manifest. Runs a separate in-pipeline job using add_ssh_keys with the explicit cataloged fingerprints — the checkout/deploy key is never materialised. On by default; pass --ssh-keys=false to skip SSH key extraction (e.g. when running env-var capture only). (default true)
  ...
  -y, --yes                           Acknowledge an unattended capture-all when neither --context nor --project is given. Without this (or --no-input) a non-interactive run that would sweep EVERY context/project with values fails closed instead of triggering real extraction pipelines.
```
